...
> 实现状态：Windows 已具备共享两槽快捷键语义、双击状态机、HKCU 本机设置、最大化/全屏前台检测、快捷键与剪贴板记录保护及可组合暂停原因；macOS 仅复用共享接口、状态机、ViewModel 与 UI 语义，未宣称原生能力完成
