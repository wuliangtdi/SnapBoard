...
- Blob 写入采用临时文件、原子重命名和数据库事务，启动时清理孤儿文件。
...
当前已实现每用户单实例及命名管道激活、托盘后台生命周期、按需创建并释放主/快速/设置窗口、原生全局快捷键、冲突回滚、恢复默认、Per-Monitor V2 定位、前台目标保存、开机启动配置、暂停记录和明确退出。Windows 实机已验证第二实例复用同一进程、后台启动、快速/设置窗口、普通窗口焦点恢复和退出；原生热键冲突由两个真实 message-only window 验证。托盘菜单点击、物理热键按键、多显示器/DPI、真实 HKCU 开机启动和 8 小时常驻仍待交互验收，因此组合条目保持部分完成。
...
- [ ] 建立版本化数据库 Schema 和迁移机制。
- [ ] 实现单写队列、Repository、事务和恢复策略。
- [ ] 实现 Blob 存储、缩略图、引用计数和孤儿清理。
- [ ] 实现 FTS5 中文/英文/代码检索和稳定分页。
- [ ] 实现去重、置顶、标签、删除、清空和保留策略。
- [ ] 实现应用黑名单、敏感格式和大小限制责任链。
- [ ] 实现数据库损坏备份和可诊断的恢复流程。
