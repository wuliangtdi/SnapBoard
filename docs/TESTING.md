...
完整直接依赖检查仅剩 SkiaSharp 3.119.4 -> 4.151.0 和 SQLitePCLRaw.bundle_e_sqlite3 2.1.12 -> 3.0.5。两者本轮明确保留：Avalonia.Skia 12.1.1 仍声明 SkiaSharp 3.119.4，Microsoft.Data.Sqlite 10.0.10 仍声明 SQLitePCLRaw 2.1.11 系列；跨越两条原生/AOT 链的大版本会脱离上游已验证组合。SDK 10.0.302 与仓库工具 Velopack 1.2.0 均为当前稳定版。

GitHub CI run 30696518333 在提交 16088f0 上完成；更新后的 checkout/setup-dotnet 已在 Windows、macOS arm64 和 macOS Intel runner 上真实运行，三个平台构建/测试与 win-x64、osx-arm64、osx-x64 Native AOT 六个 job 最终全绿。首次 macOS Intel 尝试中 `SqliteClipboardHistoryStoreTests.SQLiteFtsSearchHonorsCancellationDuringRealQuery` 因查询先于取消完成而未抛 `OperationCanceledException`，只重跑失败 job 后通过，没有代码或标签变更；该性能时序偶发风险保留记录。v0.1.4 Release run 30696534024 的四个 job 全绿，公开 Release 为非草稿、非预发布，共 31 个 uploaded 附件，覆盖 Windows MSI/ZIP、两个 macOS 架构的 DMG/PKG、Velopack 包和已签名更新源。
