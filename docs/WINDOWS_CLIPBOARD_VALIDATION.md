...
`SnapBoard.Platform.Windows.Tests` 共 52 项并全部通过，区分纯逻辑测试和真实 Windows 原生集成测试。覆盖监听器生命周期、取消、序列号去重、有限退避、队列溢出、Unicode/ANSI Text、HTML、RTF、DIB/DIBV5/注册 PNG、File List、格式清单、事件时 owner/foreground PID、来源 EXE/AUMID/Package Family、自定义来源标记、反馈抑制、`INPUT` ABI、UIPI 结构化降级、SendInput 前 HWND/PID 与前台窗口二次校验、热键冲突回滚、两个真实 message-only window 的注册冲突、开机启动配置、CurrentUserOnly 单实例命名管道、Codex/截图工具真实 AppsFolder 名称与图标/GDI 资源释放，以及 Credential Manager 的真实临时密钥往返和错误状态映射。

`SnapBoard.Desktop.HeadlessTests` 共 28 项并全部通过，覆盖正式历史的每页增量加载、旧搜索取消、真实内容写回请求、来源元数据单次异步解析、10,000 次历史变化合并为一次静默期刷新、采集持久化失败隔离和异常消息脱敏。完整解决方案共 150 项：142 项通过、8 项仅限 macOS 原生环境的测试跳过、0 项失败；Application 8/8、Infrastructure 19/19、Windows 52/52、Desktop Headless 28/28。
...
五轮完成的 10,000 次平台压力测试功能正确性均通过：无死锁，10,000 个正常自写事件均被观察到，反馈循环和监听器/写入器 Channel 丢弃均为 0。前四轮资源增长未达预算，退出竞争进程后的最新干净样本达到该隔离探针预算。
...
| 5 | 100 | 10,000/10,000 | 0 | 270 | 0 | 0 | 0.06% | 7.38 MiB | -3 | 通过 |

因此，Phase 1.3 的“无死锁、正常事件不丢失、不产生无限自复制”功能退出条件和隔离平台探针的 8 MiB 预算已验证。测试期间发现旧 AOT 桌面进程因每个事件排队一次完整历史刷新而增长到 7.24 GB Private Bytes；当前代码已用单个可复用 150 ms 定时器合并刷新，并以 10,000 次 Headless 事件验证只发生一次静默期刷新。修复后的完整 AOT 桌面端到端压力和 8 小时长稳仍未完成，不能把平台探针结果外推为整体资源门槛通过。
...
| Microsoft Store 应用来源 | Codex | 自动原生通过，手动待复核 | 从当前运行的真实 Codex 进程读取到 AUMID 与 Package Family，并从 AppsFolder 得到 Codex 名称和非空图标像素；用户报告中的实际复制发生在修复前，新构建尚未重新手动复制验收 |
| 截图图片与来源 | 截图工具 | 自动原生通过，手动待复核 | 当前已安装截图工具的 AppsFolder 本地化名称/图标通过，注册 `PNG` 格式原生写回/读取往返通过；尚未在新构建中实际截图并核对历史图片、名称和图标 |
...
| 项目 | 结果 | 实际证据/限制 |
| --- | --- | --- |
| Schema 与迁移 | 通过 | v1-v5 首次建库、逐版本升级和重复初始化通过；v5 保存来源 AUMID、Package Family 和归属依据，WAL、外键、busy timeout 由真实 SQLite 连接断言 |
...
| 来源应用元数据 | 通过（自动测试） | `WM_CLIPBOARDUPDATE` 只快照 PID，读取时校验序列并按 owner、事件时 foreground、读取时 owner 降级；EXE/AUMID/Package Family/归属依据均显式投影，AppsFolder 与传统 Shell 图标转换为 32 x 32 BGRA；真实 Codex/截图工具图标通过，连续 64 次无超预算 GDI Object 增长 |
| Credential Manager | 通过 | 真实临时项新增、读取、覆盖、删除和不存在通过；拒绝/取消/无会话、无效名称和超限输入映射通过，临时项最终删除 |

正式主窗口和快速窗口已接入 Application 查询用例，使用 50 条稳定分页、`VirtualizingStackPanel`、150 ms 搜索防抖、可取消查询和结果代际检查；高频 `HistoryChanged` 复用一个 150 ms 定时器，只在静默期刷新。原图、正文和缩略图均按需读取，图片解码在后台执行。来源名称/图标也只在虚拟化项目进入视口时异步解析，UI 线程不读取 EXE 或调用 Shell。正式组合根不创建示例历史；parameterless ViewModel 中的样例只服务 XAML 设计和无数据库视觉测试。

100,000 条生成数据平均 554.7 字符，Schema v5 来源身份投影接入后重新导入耗时 31,113.58 ms；中文/英文/代码共 300 次搜索总体 P95 2.37 ms、最大 7.49 ms，达到 P95 `< 80 ms` 且不超过 200 ms 的门槛。数据库为 515,645,440 字节；该结果只适用于当前 Windows x64 主机。

## 6. Native AOT 与进程资源

当前 `win-x64` self-contained Native AOT `SnapBoard.Desktop.exe` 为 29,531,648 字节，SHA-256 为 `F712C3D312F0AE60AEFCF669001AC03ED279E11A51AA2EADA7DDA71BD7AD6E36`。发布输出 0 个 AOT/裁剪警告，目录无 `coreclr.dll`/`clrjit.dll`；原生 EXE 单次在 413.22 ms 内创建主窗口，并通过第二实例 `--exit` 退出，无残留进程。下表三轮数据来自前一最终历史构建，继续作为可重复进程基线，不由本次单次冒烟替换。
...
另一次关闭窗口托盘采样实际持续 1,152.83 秒（19 分 12.83 秒）：最终 PWS 88.29 MiB、Private Bytes 120.99 MiB、句柄 1295，后台平均 CPU 0.000%。它完成了至少 10 分钟的观察，但不是 8 小时长稳；Private Bytes 仍超过 100 MiB。最新隔离平台 10,000 次压力增长 7.38 MiB，但完整 AOT 桌面尚未在刷新合并修复后重跑，因此整体资源门槛保持未完成。

## 7. 待验收

- 物理全局快捷键、托盘菜单点击、真实 HKCU 开机启动、多显示器、混合 DPI、睡眠唤醒和 8 小时后台常驻。
- Explorer 的单文件、多文件和目录 `CF_HDROP`。
- 在新构建上复核 Codex 实际文字复制，以及截图工具实际截图的图片、来源名称和图标；自动原生身份/像素/PNG 往返不能替代该项。
- Chrome/Edge/Firefox 的正文、地址栏、HTML、图片复制与写回。
- 经用户明确允许后启动高完整性目标，验证 UIPI 返回“已复制，请手动粘贴”。
- Office 和远程桌面只在可隔离的生成数据或测试会话中执行。
- 使用隔离数据目录对修复后的完整 AOT 桌面重跑 10,000 次压力，确认刷新合并后的 Private Bytes、句柄和 UI 响应；执行 8 小时长稳。
